...
function placeMapPointer(position) {
  ...
  setTimeout(function () {
    new TWEEN.Tween(camera.position).to(originalCameraPosition, duration).easing(TWEEN.Easing.Quadratic.InOut).start();
  }, 3000);
}
// Get all checkboxes
...
//   alert(playerName);
  ...
  onValue(ppp, (snapshot) => {
    ...
    if (!player) {
      gltfLoader.load("./character/michelle.glb", function (gltf) {
        ...
        createUltimate2(gltf.animations);
        console.log(latestUserPosition);
        player.position.set(latestUserPosition.x, 0, latestUserPosition.z + 6);
  ...
  onValue(friendsList, (snapshot) => {
    ...
    console.log(data);
    // Store the existing friend IDs in an array
    const existingFriendIds = Object.keys(friendModels);
    console.log(existingFriendIds);
    if (data) {
      ...
      friendIds.forEach((friendId) => {
        ...
        console.log("hahahahahhahahahahha");
        onValue(ppp, (snapshot) => {
          ...
          if (friendData) {
            ...
            if (friendModels[friendId]) {
              ...
              const model = friendModels[friendId].model;
              const label = friendModels[friendId].label;
              scene.remove(model);
              scene.remove(label);
              delete friendModels[friendId];
            }
          }
        });
      });
  ...
  ambientLight = new THREE.HemisphereLight("white", "darkslategrey", 3);

  new RGBELoader().load("./cloudysky.hdr", function (texture) {
  ...
  if (playerName == null) p.textContent = "You";
  //p.style.color = 'white';
  ...
  cPointLabel.element.style.textShadow = "-1px -1px 0 black, 1px -1px 0 black, -1px 1px 0 black, 1px 1px 0 black";
...
function animate() {
  ...
  if (isCodeExecutionEnabled) {
    // Make sure characterMesh is defined
    movePlayer();
    moveNpc();
    ...
    if (yyy.x > rooms[currentRoom].topX) controls.setPosition(rooms[currentRoom].topX, yyy.y, yyy.z);
    if (yyy.x < rooms[currentRoom].bottomX) controls.setPosition(rooms[currentRoom].bottomX, yyy.y, yyy.z);
    if (yyy.z > rooms[currentRoom].topZ) controls.setPosition(yyy.x, yyy.y, rooms[currentRoom].topZ);
    if (yyy.z < rooms[currentRoom].bottomZ) controls.setPosition(yyy.x, yyy.y, rooms[currentRoom].bottomZ);
  }
  if (mapPointer) {
