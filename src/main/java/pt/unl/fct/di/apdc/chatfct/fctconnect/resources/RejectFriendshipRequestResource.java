...
@Path("/rejectfriendshiprequest")
@Produces(MediaType.APPLICATION_JSON + ";charset=utf-8")
public class RejectFriendshipRequestResource {

    private static final Logger LOG = Logger.getLogger(RejectFriendshipRequestResource.class.getName());
    ...
    public RejectFriendshipRequestResource() {
    }
    ...
    public Response doRejectFriendShipRequest(RejectFriendshipRequestData data, @Context HttpHeaders headers, @Context HttpServletRequest request) {
        LOG.fine("User attempt to reject friendship request");
        ...
        try {
            ...
            final Response postRejectFriendshipRequestDBRequest = RestClientUtils.postRejectFriendshipRequest(data);
            if (postRejectFriendshipRequestDBRequest.getStatus() == Response.Status.OK.getStatusCode()) {
                txn.commit();
                LOG.fine("Friendship request was rejected");
            } else {
                txn.rollback();
                LOG.fine("Server Error");
            }
            return postRejectFriendshipRequestDBRequest;
        } catch (Exception e) {
            txn.rollback();
    ...
    private Response checkData(RejectFriendshipRequestData data, String username) {
