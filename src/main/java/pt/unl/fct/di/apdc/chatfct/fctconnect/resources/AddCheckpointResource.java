...
public class AddCheckpointResource {

    private static final Logger LOG = Logger.getLogger(AddCheckpointResource.class.getName());
    ...
    public Response doAddCheckpoint(AddCheckpointData data, @Context HttpHeaders headers, @Context HttpServletRequest request) {
        LOG.fine("User attempt to add checkpoint");
        ...
        try {
            ...
            final Response postCheckpointDBRequest = RestClientUtils.postCheckpoint(data);
            if (postCheckpointDBRequest.getStatus() == Response.Status.OK.getStatusCode()) {
                txn.commit();
                LOG.fine("Checkpoint was added");
            } else {
                txn.rollback();
                LOG.fine("Server Error");
            }
            return postCheckpointDBRequest;
        } catch (Exception e) {
            txn.rollback();
    ...
    private Response checkData(AddCheckpointData data, String username) {
